...
| **MariaDB**    | 10.4-10.6     | MySQL 互換       | MySQL 代替           |
| **Oracle**     | 12c, 19c, 21c | エンタープライズ | 既存 Oracle 環境     |
| **SQL Server** | 2017-2022     | Microsoft        | Windows 環境         |
...
クラウドネイティブなリレーショナルデータベース。MySQL/PostgreSQL 互換。
...
フルマネージド NoSQL データベース。高性能、自動スケーリング。
...
MongoDB 互換のドキュメントデータベース。フルマネージド。
...
フルマネージドグラフデータベース。Apache TinkerPop Gremlin、SPARQL 対応。
...
ペタバイト規模のデータウェアハウス。列指向ストレージ、MPP 処理。
...
インメモリキャッシュサービス。Redis、Memcached 対応。
...
_次のセクション: [04. ネットワーキング](./04-networking.md)_
