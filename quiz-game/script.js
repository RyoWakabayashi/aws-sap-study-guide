...
class QuizGame {
    ...
    showAllQuestions() {
        console.table(allQuestions.map(q => ({
            id: q.id,
            category: q.category,
    ...
    bindEvents() {
        ...
        this.backToResultBtn.addEventListener('click', () => this.showScreen('result-screen'));
    }
    ...
    startQuiz() {
        ...
        console.log(`✅ Questions loaded: ${allQuestions.length} questions available`);
        
        this.resetQuiz();
        this.currentQuestions = this.getRandomQuestions(this.totalQuestions);
        
        if (this.debugMode) {
            console.log('🎯 Selected questions for this quiz:', this.currentQuestions.map(q => ({
                id: q.id,
                category: q.category,
                question: q.question.substring(0, 50) + '...'
            })));
    ...
    showResults() {
        ...
        this.performanceMessage.innerHTML = this.getPerformanceMessage(accuracyPercent);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (typeof allQuestions !== 'undefined') {
        ...
        const invalidQuestions = allQuestions.filter(q => 
            !q.id || !q.category || !q.question || !q.options || 
        ...
        console.error('❌ allQuestions is not defined. Check if questions.js is loaded properly.');
    }
