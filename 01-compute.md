...
| タイプ | 特徴 | 用途 | vCPU:メモリ比 |
|--------|------|------|---------------|
| **c5/c6i** | 高性能プロセッサ | CPU集約的処理 | 1:2 |
| **c6g** | ARM Graviton2 | 高性能・低コスト | 1:2 |

#### メモリ最適化
| タイプ | 特徴 | 用途 | vCPU:メモリ比 |
|--------|------|------|---------------|
| **r5/r6i** | 高メモリ | インメモリDB | 1:8 |
| **x1e** | 超高メモリ | SAP HANA | 1:16 |
| **z1d** | 高周波数CPU | 単一スレッド性能 | 1:8 |

#### ストレージ最適化
| タイプ | 特徴 | 用途 | ストレージ |
|--------|------|------|-----------|
| **i3/i4i** | NVMe SSD | 高IOPS | インスタンスストア |
...
### 概要
Dockerコンテナのオーケストレーションサービス。AWS独自のコンテナ管理サービス。
...
### 概要
マネージドKubernetesサービス。Kubernetesコントロールプレーンを自動管理。
...
|--------|-----------|-------------|--------|------|
| **マネージド** | AWS管理 | 制限あり | 中 | 一般的用途 |
| **セルフマネージド** | ユーザー管理 | 自由 | 低 | 特殊要件 |
| **Fargate** | AWS管理 | 制限あり | 高 | サーバーレス |
...
#### リソース制限
```
メモリ: 128MB - 10,240MB (1MB単位)
CPU: メモリに比例 (1,769MBで1vCPU)
実行時間: 最大15分
...
3. **スケーラビリティ**: Auto Scalingの適切な設定
4. **可用性**: Multi-AZ、複数インスタンスタイプの活用
5. **セキュリティ**: IAMロール、セキュリティグループの適切な設定
...
*次のセクション: [02. ストレージサービス](./02-storage.md)*
